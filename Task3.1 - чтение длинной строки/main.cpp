  ...
  while(true)
    {
        ...
        fgets(readStart, size-(readStart-buf)), str); /* передаём место, с которого продолжаем читать,
        ...
        }else
        {
            ...
            readStart = buf + size - 1; // переопределяем значение метки, с которой продолжаем считывать текст
            size *= 2;
            buf[size-1]=123;
        }
